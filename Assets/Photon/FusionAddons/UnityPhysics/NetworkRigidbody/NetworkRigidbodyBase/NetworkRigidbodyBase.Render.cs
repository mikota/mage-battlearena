        ...
        if (syncParent) {
          ...
          if (frData.TRSPData.Parent != default) {
            ...
            if (frHasNonNetworkedParent || frData.TRSPData.Parent != toData.TRSPData.Parent) {
              ...
              if (useWorldSpace) {
                tr.SetPositionAndRotation(frData.TRSPData.Position, frData.TRSPData.Rotation);
              } else {
                tr.localPosition = frData.TRSPData.Position;
                tr.localRotation = frData.TRSPData.Rotation;
              }
              if (syncScale) {
